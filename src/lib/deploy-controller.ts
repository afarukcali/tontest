...
class JettonDeployController {
  ...
  async transfer(tonConnection: TonConnection, amount: BN, toAddress: string, fromAddress: string, ownerJettonWallet: string) {
    const { address } = await tonConnection.connect();
    const tc = await getClient();
  ...
  async getJettonDetails(contractAddr: Address, owner: Address, tonConnection: TonConnection) {
    ...
    const jWalletAddress = await makeGetCall(contractAddr, "get_wallet_address", [beginCell().storeAddress(owner).endCell()], ([addressCell]) => cellToAddress(addressCell), tc);
