...
const sha256 = (str: string) => {
  ...
  sha.update(str);
  return buf.Buffer.from(sha.digestSync());
};
...
export function buildJettonOnchainMetadata(data: { [s: string]: string | undefined }): Cell {
  ...
  Object.entries(data).forEach(([k, v]: [string, string | undefined]) => {
    ...
    console.log("v", v);
    console.log("jettonOnChainMetadataSpec[k as JettonMetaDataKeys]", jettonOnChainMetadataSpec[k as JettonMetaDataKeys]);
    let bufferToStore = buf.Buffer.from(v, jettonOnChainMetadataSpec[k as JettonMetaDataKeys]);
...
async function parseJettonOffchainMetadata(contentSlice: Slice): Promise<{
  ...
  const jsonURI = contentSlice.readRemainingBytes().toString("ascii").replace("ipfs://", "https://ipfs.io/ipfs/");
...
export function initData(owner: Address, data?: { [s in JettonMetaDataKeys]?: string | undefined }, offchainUri?: string) {
  if (!data && !offchainUri) {
...
export function mintBody(owner: Address, jettonValue: BN, transferToJWallet: BN, queryId: number): Cell {
  return beginCell()
